...
describe("CREESTLUpgradeable token", function () {
    async function deploys() {
        ...
        let tokenUpgradeableTx = await ethers.getContractFactory(
            "CRSTLUpgradeable"
        );
        let tokenUpgradeable = await upgrades.deployProxy(
            tokenUpgradeableTx,
    ...
    describe("Deployment", async function () {
        it("OK: Have correct name, symbol, decimals", async function () {
            ...
            expect(await tokenUpgradeable.name()).to.equal(
                "CREESTLUpgradeable"
            );
    ...
    describe("Transactions", async function () {
        it("OK: Transfer tokenUpgradeables between accounts", async function () {
            ...
            let endBalance = await tokenUpgradeable.balanceOf(
                clientAcc1.address
            );
        ...
        it("Revert: Sender doesn't have enough tokenUpgradeables", async function () {
            let { tokenUpgradeable } = await loadFixture(deploys);
            let startBalance = await tokenUpgradeable.balanceOf(
                ownerAcc.address
            );
            let transferAmount = parseEther("100");
            await expect(
        ...
        it("OK: update balances after transfers", async function () {
            let { tokenUpgradeable } = await loadFixture(deploys);
            let startOwnerBalance = await tokenUpgradeable.balanceOf(
                ownerAcc.address
            );
            ...
            let endOwnerBalance = await tokenUpgradeable.balanceOf(
                ownerAcc.address
            );
            ...
            let endClientBalance1 = await tokenUpgradeable.balanceOf(
                clientAcc1.address
            );
            expect(endClientBalance1).to.equal(transferAmount1);

            let endClientBalance2 = await tokenUpgradeable.balanceOf(
                clientAcc2.address
            );
    ...
    describe("Upgrades", () => {
        it("OK: Have a new method after upgrade", async () => {
            let tokenUpgradeableV1Tx = await ethers.getContractFactory(
                "CRSTLUpgradeable"
            );
            let tokenUpgradeableV2Tx = await ethers.getContractFactory(
                "CRSTLUpgradeableV2"
            );

            let tokenUpgradeableV1 = await upgrades.deployProxy(
                tokenUpgradeableV1Tx,
                ["CREESTLUpgradeable", "CRSTLU", 18],
                {
                    initializer: "initialize",
                    kind: "uups",
